...
int checkAllSet(int *set, int size);
...
void setRoutingTable(GRAPH* network, int*** table) {
	int i, j, size=network->size, *set, tmp_id, tmp_cost, fixed;
	NODE *tmp;

	set = (int*)calloc(size, sizeof(int));

	while(checkAllSet(set, size) != 1){
		for(i=0; i<size; i++){
			if(set[i] == 0){
				fixed = 1;
				tmp = network->adj_list[i];
				while(tmp != 0){
					...
					for(j=0; j<size; j++){
						if(i==j) continue;
						if(tmp_id == j) continue;
						if(tmp_cost+table[tmp_id][j][COST] > table[i][j][COST] && table[i][j][NEXT] != tmp_id) continue;
						if(tmp_cost+table[tmp_id][j][COST] == table[i][j][COST] && tmp_id >= table[i][j][NEXT]) continue;
						table[i][j][NEXT] = tmp_id;
						table[i][j][COST] = tmp_cost+table[tmp_id][j][COST];
						fixed = 0;
					}
					tmp = tmp->next;
				}
